...
MAC_CELEBRATION_AUDIO_SCRIPT = CONSOLE_DIR / "scripts" / "play_mac_celebration_audio.py"
MAC_AUDIO_HELPER_SCRIPT = CONSOLE_DIR / "scripts" / "mac_audio_helper.swift"
MAC_CELEBRATION_AUDIO_DIR = CONSOLE_DIR / "runtime" / "mac-audio"
MAC_AUDIO_HELPER_BINARY = MAC_CELEBRATION_AUDIO_DIR / "mac_audio_helper"
MAC_AUDIO_KEEPALIVE_PATH = MAC_CELEBRATION_AUDIO_DIR / "keepalive-silence.wav"
MAC_AUDIO_HELPER_PORT = int(os.environ.get("MIRA_SHENZHEN_MAC_AUDIO_HELPER_PORT", "18777"))
MAC_CELEBRATION_SOURCE_AUDIO = Path(
    os.environ.get(
        "MIRA_SHENZHEN_CELEBRATION_AUDIO_SOURCE",
        REPO_ROOT / "Mira-Light-Voice-Cloud-Ready" / "docs" / "0525" / "happy.mp3",
    )
).expanduser().resolve()
MAC_CELEBRATION_SOURCE_SECONDS = float(os.environ.get("MIRA_SHENZHEN_CELEBRATION_AUDIO_SECONDS", "17.0"))
...
def mac_celebration_audio_path(profile: str) -> Path:
    return MAC_CELEBRATION_AUDIO_DIR / f"{profile}.wav"


def build_mac_celebration_audio(profile: str) -> Path:
    if not MAC_CELEBRATION_AUDIO_SCRIPT.is_file():
        raise FileNotFoundError(f"Mac celebration audio script not found: {MAC_CELEBRATION_AUDIO_SCRIPT}")
    target = mac_celebration_audio_path(profile)
    if profile == "celebrate" and MAC_CELEBRATION_SOURCE_AUDIO.is_file():
        if (
            target.is_file()
            and target.stat().st_size > 0
            and target.stat().st_mtime >= MAC_CELEBRATION_SOURCE_AUDIO.stat().st_mtime
        ):
            return target
        ffmpeg = shutil.which("ffmpeg")
        if not ffmpeg:
            raise RuntimeError("ffmpeg is required to trim the configured celebration audio source")
        ...
        result = run_local_command(
            [
                ...
                "error",
                "-i",
                str(MAC_CELEBRATION_SOURCE_AUDIO),
                "-t",
                f"{MAC_CELEBRATION_SOURCE_SECONDS:.3f}",
                "-ar",
                "44100",
                "-ac",
                "2",
        ...
        if result.returncode != 0:
            raise RuntimeError(result.stderr or result.stdout or "Failed to trim celebration audio source")
        return target
    if (
        target.is_file()
        and target.stat().st_size > 0
        and target.stat().st_mtime >= MAC_CELEBRATION_AUDIO_SCRIPT.stat().st_mtime
    ):
        return target
    command = [
        os.environ.get("PYTHON", "python3"),
        str(MAC_CELEBRATION_AUDIO_SCRIPT),
        "--profile",
        profile,
        "--output",
        str(target),
    ...
    result = run_local_command(command, timeout_seconds=5.0)
    if result.returncode != 0:
        raise RuntimeError(result.stderr or result.stdout or "Failed to build Mac celebration audio")
    return target


def build_mac_audio_keepalive_file() -> Path:
    if MAC_AUDIO_KEEPALIVE_PATH.is_file() and MAC_AUDIO_KEEPALIVE_PATH.stat().st_size > 0:
        return MAC_AUDIO_KEEPALIVE_PATH
    command = [
        os.environ.get("PYTHON", "python3"),
        str(MAC_CELEBRATION_AUDIO_SCRIPT),
        "--profile",
        "silence",
        "--output",
        str(MAC_AUDIO_KEEPALIVE_PATH),
        "--build-only",
    ]
    result = run_local_command(command, timeout_seconds=5.0)
    if result.returncode != 0:
        raise RuntimeError(result.stderr or result.stdout or "Failed to build Mac audio keepalive file")
    return MAC_AUDIO_KEEPALIVE_PATH


def start_mac_audio_keepalive() -> None:
    afplay = shutil.which("afplay")
    if not afplay:
        print("[shenzhen-console] mac audio keepalive skipped: afplay is unavailable")
        return
    try:
        target = build_mac_audio_keepalive_file()
        subprocess.run(["pkill", "-f", "mira-light-audio-keepalive"], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        command = (
            f"exec -a mira-light-audio-keepalive /bin/sh -c "
            f"'{shlex.quote(afplay)} {shlex.quote(str(target))}; "
            f"while true; do {shlex.quote(afplay)} {shlex.quote(str(target))}; done'"
        )
        subprocess.Popen(
            ["/bin/bash", "-lc", command],
        ...
        print(f"[shenzhen-console] mac audio keepalive started: {target}")
    except Exception as exc:  # noqa: BLE001 - keep the console alive if audio warmup fails.
        print(f"[shenzhen-console] mac audio keepalive failed: {exc}")


def send_mac_audio_helper_command(command: str, timeout_seconds: float = 0.12) -> str | None:
    try:
        with socket.create_connection(("127.0.0.1", MAC_AUDIO_HELPER_PORT), timeout=timeout_seconds) as sock:
...
def build_mac_audio_helper_binary() -> Path | None:
    if not MAC_AUDIO_HELPER_SCRIPT.is_file():
        print(f"[shenzhen-console] mac audio helper missing: {MAC_AUDIO_HELPER_SCRIPT}")
        return None
    if (
        MAC_AUDIO_HELPER_BINARY.is_file()
        and MAC_AUDIO_HELPER_BINARY.stat().st_size > 0
        and MAC_AUDIO_HELPER_BINARY.stat().st_mtime >= MAC_AUDIO_HELPER_SCRIPT.stat().st_mtime
    ):
        return MAC_AUDIO_HELPER_BINARY
    swiftc = shutil.which("swiftc")
    if not swiftc:
        print("[shenzhen-console] mac audio helper skipped: swiftc is unavailable")
        return None
    MAC_AUDIO_HELPER_BINARY.parent.mkdir(parents=True, exist_ok=True)
    result = run_local_command(
        [swiftc, str(MAC_AUDIO_HELPER_SCRIPT), "-o", str(MAC_AUDIO_HELPER_BINARY)],
        timeout_seconds=20.0,
    )
    if result.returncode != 0:
        print(f"[shenzhen-console] mac audio helper compile failed: {result.stderr or result.stdout}")
        return None
    return MAC_AUDIO_HELPER_BINARY


def start_mac_audio_helper() -> bool:
    if send_mac_audio_helper_command("ping\n", timeout_seconds=0.05) == "OK pong":
        return True
    helper_binary = build_mac_audio_helper_binary()
    if helper_binary is None:
        return False

    try:
        celebrate = build_mac_celebration_audio("celebrate")
        click = build_mac_celebration_audio("click")
        keepalive = build_mac_audio_keepalive_file()
        subprocess.run(
            ["pkill", "-f", str(MAC_AUDIO_HELPER_BINARY)],
            check=False,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
        ...
        while time.monotonic() < deadline:
            if send_mac_audio_helper_command("ping\n", timeout_seconds=0.08) == "OK pong":
                subprocess.run(
                    ["pkill", "-f", "mira-light-audio-keepalive"],
                    check=False,
                    stdout=subprocess.DEVNULL,
                    stderr=subprocess.DEVNULL,
                )
                print(f"[shenzhen-console] mac audio helper ready on 127.0.0.1:{MAC_AUDIO_HELPER_PORT}")
                return True
            time.sleep(0.08)
    except Exception as exc:  # noqa: BLE001 - keep the console alive if helper startup fails.
        print(f"[shenzhen-console] mac audio helper failed: {exc}")
    return False


def warm_mac_celebration_audio() -> None:
    try:
        for profile in ("celebrate", "click"):
            target = build_mac_celebration_audio(profile)
            print(f"[shenzhen-console] mac audio ready {profile}: {target}")
        if not start_mac_audio_helper():
            start_mac_audio_keepalive()
    except Exception as exc:  # noqa: BLE001 - audio fallback must not block the console.
        print(f"[shenzhen-console] mac audio warmup failed: {exc}")


def start_mac_celebration_audio(profile: str = "celebrate") -> dict:
    target = build_mac_celebration_audio(profile)
    helper_result = send_mac_audio_helper_command(f"play {profile}\n", timeout_seconds=0.2)
    if helper_result == "OK playing":
        return {"mode": "helper", "file": str(target), "helperPort": MAC_AUDIO_HELPER_PORT}

    afplay = shutil.which("afplay")
    if not afplay:
        raise RuntimeError("afplay is not available on this machine")
    command = [
        afplay,
        str(target),
    ]
    ...
    return {"pid": process.pid, "file": str(target), "command": command_preview(command)}
...
def _run_remote_script_unlocked(
    *,
    ...
    if password and os.name == "nt" and plink_bin:
        ...
        plink_hostkey = os.environ.get("MIRA_SHENZHEN_PLINK_HOSTKEY", "").strip()
        if not plink_hostkey:
            plink_hostkey = DEFAULT_PLINK_HOSTKEYS.get((host, port), "")
        if plink_hostkey:
            command.extend(["-hostkey", plink_hostkey])
...
class ShenzhenConsoleHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            if path == "/api/mac-audio/celebrate":
                profile = str(body.get("profile") or "celebrate")
                if profile not in {"celebrate", "click"}:
                    self._send_json(400, {"ok": False, "error": "Unknown Mac audio profile"})
                    return
                audio = start_mac_celebration_audio(profile=profile)
                self._send_json(202, {"ok": True, "audio": audio})
                return
...
def main() -> int:
    ...
    Thread(target=warm_mac_celebration_audio, daemon=True).start()
    if password:
        Thread(
