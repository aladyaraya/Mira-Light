...
const TOKEN_TOTAL = 1000000000000;
const CELEBRATE_SCENE_ID = "04_offer_celebrate";
const CELEBRATE_API_URL = `/api/run/${CELEBRATE_SCENE_ID}`;
const IMMEDIATE_LIGHT_API_URL = "/api/quick-action/led_rainbow_spin";
const MAC_AUDIO_API_URL = "/api/mac-audio/celebrate";
...
const copyByPhase = {
  idle: {
    popupMessage: "恭喜你拿到 1 万亿 Token！",
    ribbon: "恭喜达成",
    kicker: "特别奖励已送达",
    awardTitle: "1 万亿 Token 已到账",
  },
  opening: {
    popupMessage: "恭喜你拿到 1 万亿 Token！",
    ribbon: "恭喜达成",
...
async function triggerImmediatePartyLight() {
  return fetchJson(IMMEDIATE_LIGHT_API_URL, {
    method: "POST",
    body: JSON.stringify({
      source: "celebrate_immediate_feedback",
      context: {
        trigger: "accept_reward",
        feedback: "rainbow_spin",
      },
    }),
  });
}

async function triggerMacCelebrationAudio() {
  return fetchJson(MAC_AUDIO_API_URL, {
    method: "POST",
    body: JSON.stringify({
      source: "celebrate_accept_reward",
      profile: "celebrate",
...
function ensureAudioContext() {
  ...
  if (!audioContext) {
    ...
    if (typeof audioContext.createStereoPanner === "function") {
      ...
      leftBus.connect(leftPanner).connect(masterBus);
      rightBus.connect(rightPanner).connect(masterBus);
    } else {
      leftBus.connect(masterBus);
      rightBus.connect(masterBus);
    }
  }
...
async function playCelebrationAudio(runId) {
  ...
  scheduleOscTone(context, context.currentTime + 0.01, 880, 0.09, "triangle", 0, 0.2);
  ...
  fanfare.forEach((note) => {
    scheduleOscTone(context, start + note.t, note.f, note.d, "sawtooth", note.p, 0.16);
    scheduleOscTone(context, start + note.t, note.f * 0.5, note.d + 0.04, "triangle", note.p * 0.7, 0.075);
  });
  ...
  danceBass.forEach((note) => {
    scheduleOscTone(context, start + note.t, note.f, 0.2, "square", note.p, 0.14);
    scheduleOscTone(context, start + note.t, note.f * 2, 0.16, "triangle", -note.p * 0.75, 0.065);
  });

  for (let beat = 0; beat < 10; beat += 1) {
    const t = start + 1.68 + beat * 0.22;
    scheduleNoiseBurst(context, t, 0.12, beat % 2 === 0 ? -0.8 : 0.8, 0.11, 1200);
  }

  for (let clap = 0; clap < 6; clap += 1) {
    const t = start + 2.04 + clap * 0.36;
    scheduleNoiseBurst(context, t, 0.08, clap % 2 === 0 ? 0.35 : -0.35, 0.075, 2200);
  }
...
async function playScene() {
  ...
  try {
    const audioPromise = triggerMacCelebrationAudio().catch((error) => {
      console.warn("Mac celebration audio failed:", error);
    });
    await triggerImmediatePartyLight().catch((error) => {
      console.warn("Immediate lamp feedback failed:", error);
    });
