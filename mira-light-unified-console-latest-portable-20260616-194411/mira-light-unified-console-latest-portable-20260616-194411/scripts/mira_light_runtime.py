...
class MiraLightRuntime:
    ...
    def __init__(
        ...
        self._tracking_servo_velocity: dict[str, int] = {servo_name: 0 for servo_name in SERVO_KEYS}
        self._tracking_led_state: dict[str, Any] = {}
        self._book_search_step = 0
    ...
    def _clear_tracking_state(self, *, reason: str) -> None:
        with self._state_lock:
            ...
            self._book_search_step = 0
            self._tracking_servo_velocity = {servo_name: 0 for servo_name in SERVO_KEYS}
        self.log(f"[tracking] cleared reason={reason}")

    def _seed_tracking_servo_state_from_status(self) -> None:
        try:
            status_payload = self.get_client().get_status()
        except Exception as exc:  # noqa: BLE001 - tracking can still start from the last estimated state.
            self.log(f"[tracking-warning] could not seed servo state from status: {exc}")
            return
        if not isinstance(status_payload, dict):
            return
        angles = _status_to_angles(status_payload)
        ...
        for servo_name in SERVO_KEYS:
            if servo_name in angles:
                self._tracking_servo_state[servo_name] = int(angles[servo_name])
                self._tracking_servo_velocity[servo_name] = 0
    ...
    def _smooth_servo_target(
        ...
        if max_delta is not None and abs(next_value - current) > max_delta:
            next_value = current + (max_delta if next_value > current else -max_delta)
        if next_value == current and abs(target - current) > 1:
            next_value = current + (1 if target > current else -1)
        delta = int(next_value - current)
        previous_delta = int(self._tracking_servo_velocity.get(servo_name, 0))
        if max_accel is not None and delta:
            if previous_delta and delta * previous_delta < 0:
                delta = 0
            else:
                high = previous_delta + max_accel
                low = previous_delta - max_accel
                delta = max(low, min(high, delta))
            target_delta = int(target - current)
            if abs(delta) > abs(target_delta):
                delta = target_delta
            next_value = current + delta
        self._tracking_servo_state[servo_name] = int(next_value)
        self._tracking_servo_velocity[servo_name] = int(delta)
        return int(next_value)

    def _apply_tabletop_book_search_event(self, event: dict[str, Any], *, source: str) -> dict[str, Any]:
        ...
        offset3, offset4 = pattern[self._book_search_step % len(pattern)]
        self._book_search_step += 1
        desired_servo3 = max(servo3_low, min(servo3_high, servo3_neutral + offset3))
        desired_servo4 = max(servo4_low, min(servo4_high, servo4_neutral + offset4))
        payload = {
            "mode": "absolute",
            "servo3": self._smooth_servo_target("servo3", desired_servo3, alpha=0.52, max_delta=3, max_accel=1),
            "servo4": self._smooth_servo_target("servo4", desired_servo4, alpha=0.50, max_delta=2, max_accel=1),
        }
        normalized_payload = self.validate_control_payload(payload)
        led_payload = {"mode": "solid", "brightness": 132, "color": {"r": 235, "g": 240, "b": 232}}
        normalized_led = self.validate_led_payload(led_payload)

        self.log(
            "[tracking] "
            f"{source} profile=tabletop_book_search step={self._book_search_step} "
            f"-> {json.dumps(normalized_payload, ensure_ascii=False)}"
        )
        self.get_client().control(normalized_payload, move_ms=320)
        ...
        with self._state_lock:
            ...
            self._tracking_target = {
                ...
                "trackingProfile": "tabletop_book_search",
                "reason": "book_missing_search",
                "servoCommand": normalized_payload,
            }
            self._last_command = "tracking:book-search"
            self._current_step_label = "tracking:book-search"
    ...
    def apply_tracking_event(self, event: dict[str, Any], *, source: str = "vision") -> dict[str, Any]:
        ...
        was_tracking_active = self._tracking_active
        if not target_present:
            if target_mode == "tabletop_follow" or source == "vision-search":
                return self._apply_tabletop_book_search_event(event, source=source)
            self._clear_tracking_state(reason="target_missing")
            return self.get_runtime_state()
        if not was_tracking_active:
            self._seed_tracking_servo_state_from_status()
        ...
        self._book_search_step = 0
        is_tabletop = target_mode == "tabletop_follow" or target_class in {"object", "book"} or detector == "book_cover_color"
        deadband = control_hint.get("deadband_norm") if isinstance(control_hint.get("deadband_norm"), dict) else {}
        yaw_deadband = float(deadband.get("yaw", 0.035 if is_tabletop else 0.02) or 0.0)
        pitch_deadband = float(deadband.get("pitch", 0.06 if is_tabletop else 0.03) or 0.0)
        ...
        if is_tabletop:
            # Yellow-book following needs visible whole-arm participation:
            # base yaw aims left/right, lower/upper arm handle desk height and
            # reach, and the head tilts toward the cover instead of staying fixed.
            is_book_tracking = detector == "book_cover_color" or target_class == "book"
            yaw_gain = 14 if is_book_tracking else 16
            pitch_gain = 10 if is_book_tracking else 13
            desired_servo1 = max(servo1_low, min(servo1_high, round(servo1_neutral + yaw_error * yaw_gain)))
            desired_servo2 = max(
                servo2_low,
                min(servo2_high, round(servo2_neutral + (lift_intent - 0.44) * 18 + pitch_error * 4)),
            )
            desired_servo3 = max(
                servo3_low,
                min(servo3_high, round(servo3_neutral + (reach_intent - 0.52) * 72 + abs(yaw_error) * 8)),
            )
            desired_servo4 = max(servo4_low, min(servo4_high, round(servo4_neutral + 7 - pitch_error * pitch_gain)))
            smooth_alpha = 0.34 if is_book_tracking else 0.42
            max_step = {"servo1": 3, "servo2": 2, "servo3": 3, "servo4": 2} if is_book_tracking else {"servo1": 4, "servo2": 3, "servo3": 4, "servo4": 3}
            max_accel = {"servo1": 1, "servo2": 1, "servo3": 1, "servo4": 2} if is_book_tracking else {"servo1": 2, "servo2": 2, "servo3": 2, "servo4": 2}
            led_brightness = 178 if detector == "book_cover_color" else 162
            led_color = {"r": 246, "g": 248, "b": 240}
            tracking_profile = "tabletop_book" if is_book_tracking else "tabletop_object"
        else:
            ...
            smooth_alpha = 0.42
            max_step = {"servo1": None, "servo2": None, "servo3": None, "servo4": None}
            max_accel = {"servo1": 3, "servo2": 2, "servo3": 3, "servo4": 2}
            led_brightness = 166 if tracking.get("distance_band") == "near" else 150
            led_color = TRACKING_FOCUS
            tracking_profile = "person_follow"

        payload = {
            "mode": "absolute",
            "servo1": self._smooth_servo_target("servo1", desired_servo1, alpha=smooth_alpha, max_delta=max_step["servo1"], max_accel=max_accel["servo1"]),
            "servo2": self._smooth_servo_target("servo2", desired_servo2, alpha=smooth_alpha, max_delta=max_step["servo2"], max_accel=max_accel["servo2"]),
            "servo3": self._smooth_servo_target("servo3", desired_servo3, alpha=smooth_alpha, max_delta=max_step["servo3"], max_accel=max_accel["servo3"]),
            "servo4": self._smooth_servo_target("servo4", desired_servo4, alpha=smooth_alpha, max_delta=max_step["servo4"], max_accel=max_accel["servo4"]),
        }
        normalized_payload = self.validate_control_payload(payload)
        led_payload = {
            "mode": "solid",
        ...
        self.log(
            "[tracking] "
            f"{source} profile={tracking_profile} yaw={yaw_error:.2f} pitch={pitch_error:.2f} "
            f"distance={tracking.get('distance_band')} zone={tracking.get('horizontal_zone')} "
            f"-> {json.dumps(normalized_payload, ensure_ascii=False)}"
        )
        self.get_client().control(normalized_payload, move_ms=340 if tracking_profile == "tabletop_book" else None)
        ...
        with self._state_lock:
            ...
            self._tracking_target = {
                ...
                "targetSubclass": tracking.get("target_subclass"),
                ...
                "objectLockStrength": tracking.get("object_lock_strength"),
                "bookColorRatio": tracking.get("book_color_ratio"),
                "bookMatchStrength": tracking.get("book_match_strength"),
    ...
    def trigger_event(self, event_name: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if event_key in {"long_touch_comfort", "touch_comfort", "comfort_sound"}:
            asset_name = str(event_payload.get("asset_name") or "speech/touch_affection_host.aiff").strip()
            if not asset_name:
                asset_name = "speech/touch_affection_host.aiff"
            audio_result = self.audio_player.play_asset(asset_name, wait=False, allow_missing=True)
            if audio_result.get("skipped") and asset_name != "speech/touch_affection_host.aiff":
                audio_result = self.audio_player.play_asset(
                    "speech/touch_affection_host.aiff",
                    wait=False,
                    allow_missing=True,
                )
            self._last_trigger = {
                ...
                "scene": "comfort_sound",
                "audio": deepcopy(audio_result),
                "ts": self._now(),
            }
            self.log(f"[trigger] {event_key} -> comfort_sound asset={asset_name}")
            return self.get_runtime_state()

        if event_key in {"stop_comfort_sound", "comfort_sound_stop", "stop_touch_comfort"}:
            self.audio_player.stop_all()
            self._last_trigger = {
                "event": event_key,
                "payload": deepcopy(event_payload),
                "scene": "comfort_sound_stop",
                "ts": self._now(),
            }
            self.log(f"[trigger] {event_key} -> comfort_sound_stop")
            return self.get_runtime_state()
