...
def soft_white_rotation_led(offset: int = 0, brightness: int = 150) -> Step:
    pixels: List[Dict[str, int]] = []
    for index in range(LED_PIXEL_COUNT):
        phase = (index - offset) % 10
        if phase in (0, 1, 2):
            pixel_brightness = brightness
        elif phase in (3, 9):
            pixel_brightness = max(42, brightness - 58)
        else:
            pixel_brightness = 10
        pixels.append({"r": 255, "g": 255, "b": 255, "brightness": pixel_brightness})
    return led_vector(pixels)
...
SCENES: Dict[str, Dict[str, Any]] = {
    ...
    "touch_affection": {
        ...
        "notes": [
            ...
            "灯效使用白色分段旋转帧；固定协议控制台会用板端 spin 命令做连续白色旋转。"
        ],
        "tuning_notes": [
        ...
        "steps": [
            ...
            soft_white_rotation_led(offset=28, brightness=120),
            delay(220),
            pose("neutral"),
        ],
    },
