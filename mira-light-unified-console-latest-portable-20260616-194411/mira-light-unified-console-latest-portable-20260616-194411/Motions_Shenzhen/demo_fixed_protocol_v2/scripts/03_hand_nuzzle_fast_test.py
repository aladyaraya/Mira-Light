...
def build_steps(*, light_style: str, rub_cycles: int, linger_seconds: float, final_pose: str) -> list[RemoteStep]:
    ...
    steps = [
        RemoteStep("soft white rotating palm-ready light", light_command(light_style)),
    ...
    steps.extend(
        [
            ...
            pose("hand returns - wait close under white rotation", 2048, 2160, 1850, 2130, (140, 90, 76, 120)),
    ...
    if final_pose == "close":
        steps.extend(
            [
                ...
                RemoteStep("keep soft white rotation close", white_spin_command(125)),
            ]
        )
    else:
        steps.extend(
