...
echo "Mode:  chat + TTS only, action triggers disabled"
echo
...
elif [ "$BRIDGE_URL" != "$LOCAL_BRIDGE_URL" ]; then
  echo "WARNING: external bridge is not reachable at $BRIDGE_URL/health"
  echo "Safe voice mode will still start because action triggers are disabled."
  echo
else
...
echo "Starting Mira Light voice runtime in safe mode..."
