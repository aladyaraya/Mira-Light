...
This guide is for moving `Mira-Light-Voice-Full-Ready` to another Mac and
...
1. Copy the whole `Mira-Light-Voice-Full-Ready` folder to the new Mac.
...
## Switching To Continuous Conversation

The default launchers use `enter-vad`. In that mode, press Enter for each turn,
then speak; silence ends the turn automatically.

To use continuous listening instead, double-click:
...
Continuous mode is more natural, but it can accidentally hear speaker playback
or room noise. If transcription starts repeating phrases or triggering itself,
return to `enter-vad`.
...
```bash
MIRA_LIGHT_FORCE_BUILTIN_MIC=0 MIRA_LIGHT_MIC_DEVICE="Rick Beosound A1" ./Start-Chat.command
```
...
```bash
MIRA_LIGHT_FORCE_BUILTIN_MIC=0 MIRA_LIGHT_MIC_DEVICE="MacBook Pro麦克风" ./Start-Chat.command
```
...
```bash
./scripts/run_mira_realtime_voice_interaction.sh enter-vad --list-inputs
MIRA_LIGHT_FORCE_BUILTIN_MIC=0 MIRA_LIGHT_MIC_DEVICE="exact device name" ./Start-Chat.command
```
