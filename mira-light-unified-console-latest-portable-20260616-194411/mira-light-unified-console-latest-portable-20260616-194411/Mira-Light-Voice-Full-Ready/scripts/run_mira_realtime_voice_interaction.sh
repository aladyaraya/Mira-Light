...
CLAW_MODE="${MIRA_LIGHT_CLAW_MODE:-cloud}"
case "${CLAW_MODE}" in
  local)
  ...
  cloud)
    ...
    if [[ -z "${MIRA_LIGHT_LINGZHU_AUTH_AK:-}" ]]; then
      echo "Missing MIRA_LIGHT_LINGZHU_AUTH_AK." >&2
      echo "Run ./commands/Configure-Cloud.command first, or edit config/mira-light-realtime.env." >&2
      exit 2
    fi
    ;;
  auto)
...
SPEECH_ENGINE="${MIRA_LIGHT_SPEECH_ENGINE:-legacy-full}"
case "${SPEECH_ENGINE}" in
  legacy-full|legacy|local-full)
    exec "${PYTHON_BIN}" "${ROOT}/scripts/mira_realtime_voice_interaction.py" "$@"
    ;;
  stepfun-realtime|stepaudio-realtime)
    if [[ "${MIRA_LIGHT_NO_TRIGGER:-0}" == "1" ]]; then
      set -- "$@" --no-semantic-actions
    fi
    if [[ -n "${MIRA_LIGHT_INPUT_DEVICE:-${MIRA_LIGHT_MIC_DEVICE:-}}" ]]; then
      set -- "$@" --input-device "${MIRA_LIGHT_INPUT_DEVICE:-${MIRA_LIGHT_MIC_DEVICE:-}}"
    fi
    if [[ -n "${MIRA_LIGHT_DIRECTOR_URL:-}" ]]; then
      set -- "$@" --director-url "${MIRA_LIGHT_DIRECTOR_URL}"
    fi
    exec "${PYTHON_BIN}" "${ROOT}/scripts/mira_stepfun_realtime_voice_actions.py" "$@"
    ;;
  *)
    echo "Invalid MIRA_LIGHT_SPEECH_ENGINE=${SPEECH_ENGINE}; expected legacy-full or stepfun-realtime." >&2
    exit 2
    ;;
esac
