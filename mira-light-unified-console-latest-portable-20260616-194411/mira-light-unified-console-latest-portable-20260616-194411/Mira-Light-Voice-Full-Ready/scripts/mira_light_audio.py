...
class AudioCuePlayer:
    ...
    def _find_edge_tts_script(self) -> str | None:
        """Return path to speaker-edge-tts-play.py if available and network is reachable."""
        # Edge-TTS requires access to speech.platform.bing.com which is blocked in
        # some networks. Skip it for now to avoid silent TTS failures.
        return None
    ...
    def _run(self, command: list[str], *, wait: bool, description: str) -> dict[str, Any]:
        ...
        if wait:
            ...
            stdout = (
                ...
                else str(completed.stdout or "").strip()
            )
            stderr = (
    ...
    def _build_play_command(self, asset_path: Path) -> list[str]:
        ...
        if sys.platform == "win32":
            return self._build_windows_asset_play_command(asset_path)
    ...
    def _build_windows_asset_play_command(self, asset_path: Path) -> list[str]:
        path_b64 = base64.b64encode(str(asset_path).encode("utf-8")).decode("ascii")
        script = (
            ...
            "$duration = [Math]::Max([double]$media.duration, 0.5); "
            "Start-Sleep -Milliseconds ([int](($duration + 0.25) * 1000)); "
    ...
    def _build_speech_command(self, text: str, *, voice: str) -> list[str]:
        ...
        import sys
        if sys.platform == "win32":
            # Prefer edge-tts (natural Chinese voice) over System.Speech (robotic)
            edge_tts_script = self._find_edge_tts_script()
            if edge_tts_script:
                return [sys.executable or "python", edge_tts_script, text]
            # Fallback to System.Speech (robotic, but always available)
