...
DEFAULT_RDK_X5_HOST = "192.168.0.183"
...
class BusServoRuntimeConfig:
    ...
    def from_file(cls, path: Path = DEFAULT_RUNTIME_CONFIG_PATH) -> "BusServoRuntimeConfig":
        ...
        return cls(
            ...
            require_ack=bool(raw.get("requireAck", True)),
        )
...
class TcpBusServoTransport:
    ...
    def from_runtime_config(cls, config: BusServoRuntimeConfig | None = None) -> "TcpBusServoTransport":
        ...
        return cls(host=cfg.tcp_host, port=cfg.tcp_port, timeout_seconds=cfg.timeout_seconds, require_ack=cfg.require_ack)
    ...
    def send(self, command: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise BusServoTransportError(
                f"Failed to reach Mira Light bus-servo endpoint {self.host}:{self.port}: {exc.__class__.__name__}: {exc}"
            ) from exc
        raw_response = b"".join(chunks).decode("utf-8", errors="replace").strip()
        if self.require_ack and not raw_response.startswith("OK"):
            detail = raw_response or "empty response"
            raise BusServoTransportError(
                f"Mira Light bus-servo endpoint {self.host}:{self.port} did not acknowledge command {validated}: {detail}"
            )
        return {
            "ok": True,
            "host": self.host,
