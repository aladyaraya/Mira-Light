...
def default_api_system_prompt() -> str:
    return read_prompt_from_env(
        file_env="MIRA_LIGHT_LLM_SYSTEM_PROMPT_FILE",
        inline_env="MIRA_LIGHT_LLM_SYSTEM_PROMPT",
        default=DEFAULT_API_SYSTEM_PROMPT,
        root=ROOT,
    )


def default_stt_initial_prompt() -> str:
    return read_prompt_from_env(
        file_env="MIRA_LIGHT_STT_INITIAL_PROMPT_FILE",
        inline_env="MIRA_LIGHT_STT_INITIAL_PROMPT",
        default="",
        root=ROOT,
    )
...
def send_via_stepfun_messages(
    ...
    *,
    ...
    try:
        ...
        raise VoiceToClawError("Missing requests package. Run Setup-Mira-Light-Windows-Voice.ps1 first.") from exc
    ...
    if not api_key:
        raise VoiceToClawError("StepFun API key is required. Set STEPFUN_API_KEY or STEP_API_KEY.")

    endpoint = os.environ.get("STEPFUN_LLM_ENDPOINT", "https://api.stepfun.com/v1/chat/completions").strip()
    model = os.environ.get("STEPFUN_LLM_MODEL", "step-3.7-flash").strip()
    ...
    try:
        ...
        if response.status_code < 200 or response.status_code >= 300:
            raise VoiceToClawError(f"StepFun HTTP {response.status_code}: {response.text}")
        data = response.json()
    except Exception as exc:
        raise VoiceToClawError(f"StepFun API request failed: {exc}") from exc

    choices = data.get("choices")
    if not isinstance(choices, list) or not choices:
        raise VoiceToClawError("StepFun LLM returned no choices")

    first = choices[0]
    if not isinstance(first, dict):
        raise VoiceToClawError("StepFun LLM returned invalid choice format")
    ...
    if not text:
        raise VoiceToClawError("StepFun LLM returned empty reply text")
...
def send_reply(
    ...
    *,
    ...
    if args.reply_backend == "stepfun":
        text, meta = send_via_stepfun_messages(
            messages,
            timeout_seconds=args.timeout,
        )
        return text, meta, "stepfun-direct"
    ...
    try:
        ...
        try:
            ...
            if os.environ.get("STEPFUN_API_KEY") or os.environ.get("STEP_API_KEY"):
                try:
                    text, meta = send_via_stepfun_messages(
                        messages,
                        timeout_seconds=args.timeout,
                    )
                    meta["openclawError"] = f"openclaw: {exc} | newapi: {fallback_exc}"
                    return text, meta, "stepfun-direct"
                except Exception:
                    pass
            if isinstance(exc, VoiceToClawError):
                raise exc
            raise VoiceToClawError(f"openclaw-agent failed: {exc}") from exc
...
def fallback_reply_for_intent(intent: str) -> str:
    if intent.startswith("scene:"):
        ...
        return scene_reply_for_name(scene_name)
    if intent == "sigh":
        return "我在呢，先慢一点也没关系。"
    if intent == "comfort":
...
def scene_reply_for_name(scene_name: str) -> str:
    replies = {
        "celebrate": "好，我跳一下。",
        "sleep": "好，我安静下来。",
        "wake_up": "好，我醒一下。",
...
def reply_for_action_result(intent: str, trigger_result: dict[str, Any]) -> str | None:
    ...
    if trigger_result.get("skipped"):
        ...
        if reason == "cooldown-active":
            return "我刚刚动过了，先缓一下。"
        return None
    if not trigger_result.get("ok", False):
        return "我想动一下，但本地灯控 bridge 还没连上。"
    if action["type"] == "scene":
        return scene_reply_for_name(action["name"])
    return fallback_reply_for_intent(intent)
...
def calculate_rms_cv(samples: np.ndarray, sample_rate: int, frame_ms: int = 50) -> float:
    """Calculate the Coefficient of Variation (CV) of the frame RMS energy.
    
    Human speech consists of highly dynamic phonemes and pauses (high CV, typically >0.35).
    Constant background noise (fans, hums, static, crowd chatter) has constant energy (low CV).
    """
    mono = ensure_mono_float32(samples)
    if mono.size == 0:
        return 0.0
    ...
    f_rms = np.sqrt(np.mean(np.square(frames), axis=1))

    mean_rms = float(np.mean(f_rms))
    if mean_rms < 1e-6:
        return 0.0
    std_rms = float(np.std(f_rms))
...
def build_audio_metrics(samples: np.ndarray, *, sample_rate: int) -> dict[str, float]:
    ...
    rms_val = rms_level(mono)
    peak_val = peak_level(mono)
    cv_val = calculate_rms_cv(mono, sample_rate)
    return {
...
def low_energy_skip_reason(
    ...
    *,
    ...
    duration_ms = audio_metrics["durationMs"]
    rms_val = audio_metrics["rms"]
    peak_val = audio_metrics["peak"]
    cv_val = audio_metrics.get("cv", 0.0)

    # Resolve configurable gates
    skip_low_rms = getattr(args, "skip_low_rms", DEFAULT_SKIP_LOW_RMS) if args is not None else DEFAULT_SKIP_LOW_RMS
    skip_low_peak = getattr(args, "skip_low_peak", DEFAULT_SKIP_LOW_PEAK) if args is not None else DEFAULT_SKIP_LOW_PEAK
    min_speech_cv = getattr(args, "min_speech_cv", 0.35) if args is not None else 0.35
    enforce_gate_always = getattr(args, "enforce_gate_always", True) if args is not None else True

    # Optional bypass — when gate enforcement is off, accept all utterances
    if not enforce_gate_always:
        return None

    # Noise gate 1: Average energy threshold
    if rms_val < skip_low_rms:
        return f"low-rms-energy (rms={rms_val:.6f} < gate={skip_low_rms:.6f})"

    # Noise gate 2: Peak energy threshold
    if peak_val < skip_low_peak:
        return f"low-peak-energy (peak={peak_val:.6f} < gate={skip_low_peak:.6f})"

    # Noise gate 3: Temporal modulation check (filters out constant fan hum and room noise)
    if cv_val < min_speech_cv:
        return f"low-speech-cv-constant-noise (cv={cv_val:.4f} < gate={min_speech_cv:.4f})"
...
def transcribe_utterance(utterance: UtteranceResult, *, args: argparse.Namespace) -> dict[str, Any]:
    ...
    if getattr(args, "transcriber", "local-mlx") == "stepfun":
        import tempfile
        temp_wav = Path(tempfile.gettempdir()) / f"mira_stt_temp_{timestamp_slug()}.wav"
        save_wav(utterance.samples, sample_rate=int(utterance.sample_rate), path=temp_wav)
        try:
            from stepfun_asr_client import transcribe_audio_file
            api_key = os.environ.get("STEPFUN_API_KEY", "") or os.environ.get("STEP_API_KEY", "")
            endpoint = os.environ.get("STEPFUN_ASR_ENDPOINT", "https://api.stepfun.com/v1/audio/asr/sse")
            model = os.environ.get("STEPFUN_ASR_MODEL", "stepaudio-2.5-asr")
            ...
            transcript_payload = {
                ...
                "modelRepo": f"stepfun:{model}",
                "sampleRate": int(utterance.sample_rate),
            }
        finally:
            try:
                if temp_wav.exists():
    ...
    transcript_payload.setdefault("modelRepo", model_repo)
...
def run_turn(
    *,
    ...
    low_energy_reason = low_energy_skip_reason(utterance.source_meta, audio_metrics, args=args)
    ...
    # Check if there is a local shortcut action mapped to this intent
    action = action_for_intent(intent)
    trigger_result = None
    local_action_reply = None
    ...
    if action is not None:
        ...
        local_action_reply = reply_for_action_result(intent, trigger_result)

    if local_action_reply:
        reply_started = time.perf_counter()
        raw_reply_text = local_action_reply
        reply_finished = time.perf_counter()
        api_meta = {"provider": "local-action-reply", "payload": {"intent": intent, "triggerResult": trigger_result}}
        reply_backend = "local-action-reply"
        result["messages"] = []
        result["timings"]["replySeconds"] = round(reply_finished - reply_started, 3)
    else:
        # Request reply from LLM first to minimize the delay between action trigger and TTS playback
        ...
        # Trigger physical/bridge action immediately after getting LLM reply (for sound-motion sync)
        if "triggerResult" not in result:
            try:
                trigger_started = time.perf_counter()
                trigger_result = maybe_trigger_mira_action(session, intent=intent, transcript=transcript, args=args)
                trigger_finished = time.perf_counter()
    ...
    # For action commands: play TTS non-blocking (wait=False) so hardware
    # moves immediately while voice reply plays in background.
    has_action_result = (
        trigger_result is not None
        and trigger_result.get("ok")
        and trigger_result.get("action")
        and trigger_result["action"].get("name")
    )
    tts_wait = not has_action_result

    if reply_text:
        tts_started = time.perf_counter()
        audio_result = audio_player.speak_text(reply_text, voice=args.voice_mode, wait=tts_wait)
        ...
        result["postTtsCooldownSeconds"] = 0.0
        result["timings"]["postTtsCooldownSeconds"] = 0.0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--transcriber",
        choices=["local-mlx", "stepfun"],
        default="stepfun" if sys.platform == "win32" else "local-mlx",
        help="Speech-to-text transcriber backend."
    )
    ...
    parser.add_argument(
        "--initial-prompt",
        default=default_stt_initial_prompt() or None,
    ...
    parser.add_argument(
        "--reply-backend",
        choices=["openclaw-agent", "lingzhu", "stepfun"],
    ...
    parser.add_argument("--skip-low-rms", type=float, default=DEFAULT_SKIP_LOW_RMS, help="Threshold for average RMS energy. Audio below this is skipped.")
    parser.add_argument("--skip-low-peak", type=float, default=DEFAULT_SKIP_LOW_PEAK, help="Threshold for peak amplitude. Audio below this is skipped.")
    parser.add_argument("--min-speech-cv", type=float, default=0.35, help="Minimum coefficient of variation (CV) of frame RMS to classify as human speech.")
    parser.add_argument("--enforce-gate-always", action=argparse.BooleanOptionalAction, default=True, help="Enforce RMS and Peak gates regardless of audio duration.")
    ...
    write_json(
        session_dir / "session.json",
        {
            ...
            "args": {
                ...
                "vadStartMs": args.vad_start_ms,
                "vadEndMs": args.vad_end_ms,
                "skipLowRms": getattr(args, "skip_low_rms", None),
                "skipLowPeak": getattr(args, "skip_low_peak", None),
                "minSpeechCv": getattr(args, "min_speech_cv", None),
                "enforceGateAlways": getattr(args, "enforce_gate_always", None),
            },
...
def main() -> int:
    ...
    elif args.reply_backend == "stepfun":
        print(f"Reply backend: stepfun ({os.environ.get('STEPFUN_LLM_MODEL', 'step-3.7-flash')})")
    else:
